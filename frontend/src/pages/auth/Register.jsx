...
function Register() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Django/DRF typically returns field errors as { field: ["message"] }
        const firstError =
          data.detail ||
          data.message ||
  ...
  return (
    <div className="ss-auth-shell">
      <div className="ss-auth-card">
        <span className="ss-kicker">Join us</span>
        <h2 className="ss-display">Create your account</h2>
        <p className="sub">Register to save your details for faster orders.</p>

        <form onSubmit={submit}>
          <Field label="Full name" error={errors.name}>
            <input className="ss-input" type="text" placeholder="Your full name"
              value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </Field>

          <Field label="Email address" error={errors.email}>
            <input className="ss-input" type="email" placeholder="you@example.com"
              value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </Field>

          <Field label="Phone number" error={errors.phone}>
            <input className="ss-input" type="tel" placeholder="98765 43210"
          ...
          <div className="ss-row-2">
            <Field label="Password" error={errors.password}>
              <div className="ss-input-wrap">
                <input className="ss-input" type={showPw ? "text" : "password"} placeholder="Min. 6 characters"
            ...
            <Field label="Confirm password" error={errors.confirm}>
              <input className="ss-input" type={showPw ? "text" : "password"} placeholder="Re-enter password"
                value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} />
            </Field>
          </div>

          <label className="ss-check-row" style={{ marginBottom: 20, fontSize: 13 }}>
            <input type="checkbox" required /> I agree to be contacted about my orders.
          </label>

          <button type="submit" className="ss-btn ss-btn-primary ss-btn-full" disabled={loading}>
          ...
          {success && (
            <div className="ss-success-banner">
              <Check size={16} /> Account created successfully. You can now log in.
            </div>
          )}
        </form>
        ...
        <p className="ss-auth-switch">
          ...
          <a className="ss-link-btn" href="/login">Log in</a>
        </p>
      </div>
    </div>
  );
